...
- [x] 3.7.0 홈 페이지 버그 수정 및 개선
    - [x] 3.7.1 타이틀 표시 버그 수정
        - [x] 3.7.1.1 현재 타이틀 렌더링 이슈 조사 및 원인 파악 (왼쪽 정렬로 인한 겹침 확인)
        - [x] 3.7.1.2 타이틀 컴포넌트 수정 (중앙 정렬 및 스타일 개선)
        - [x] 3.7.1.3 반응형 디자인 적용
        - [x] 3.7.1.4 테스트 코드 작성 (브라우저 테스트 완료)
        - [x] 3.7.1.5 테스트 실행 및 검증
        - [x] 3.7.1.6 오류 수정 (완료)
    - [x] 3.7.2 서비스 소개 섹션 작성
        - [x] 3.7.2.1 서비스 소개 컨텐츠 기획 및 작성
        - [x] 3.7.2.2 Hero Section 디자인 및 구현 (지도 섹션 활용)
        - [x] 3.7.2.3 주요 기능 소개 카드 컴포넌트 제작 (ServiceIntro.tsx)
        - [x] 3.7.2.4 사용 방법 안내 섹션 추가
        - [x] 3.7.2.5 UI/UX 스타일링 (framer-motion 애니메이션 적용)
        - [x] 3.7.2.6 반응형 디자인 적용
        - [x] 3.7.2.7 테스트 코드 작성 (브라우저 테스트 완료)
        - [x] 3.7.2.8 테스트 실행 및 검증
        - [x] 3.7.2.9 오류 수정 (framer-motion 설치 완료)
    - [x] 3.7.3 홈 페이지 타이틀 제거 (메뉴 가림 현상 수정)
        - [x] 3.7.3.1 page.tsx에서 헤더/타이틀 섹션 제거
        - [x] 3.7.3.2 UI 확인 (메뉴 정상 표시 확인)

### New Tasks - Profile & Report Enhancements
- [x] 3.8.0 마이페이지(프로필) 기능 개선
    - [x] 3.8.1 프로필 정보 관리 UI 구현
        - [x] 3.8.1.1 이름, 연락처, 비상연락처 입력 폼 작성
        - [x] 3.8.1.2 기존 데이터 불러오기 및 표시
        - [x] 3.8.1.3 유효성 검사 (전화번호 형식 등)
    - [x] 3.8.2 프로필 저장 기능 구현
        - [x] 3.8.2.1 Supabase 사용자 테이블 업데이트 로직 작성
        - [x] 3.8.2.2 저장 성공/실패 알림 처리
        - [x] 3.8.2.3 테스트 코드 작성 및 검증

- [x] 3.9.0 신고 페이지 동행자 관리 기능 추가
    - [x] 3.9.1 동행자 정보 입력 UI 구현
        - [x] `CompanionForm` 컴포넌트 개발 (이름, 연락처, 비상연락처)
        - [x] 동행자 추가/삭제 기능 (동적 폼)
    - [x] 3.9.2 신고 데이터 구조 업데이트
        - [x] `ReportRequest` 타입에 `companions` 필드 추가
        - [x] Supabase `submit_report` RPC 연동 (SQL 업데이트 완료)
    - [x] 3.9.3 프로필 연동 강화
        - [x] '프로필에서 불러오기' 기능이 비상연락처도 포함하도록 개선 (기존 `ContactForm`에서 확인됨)