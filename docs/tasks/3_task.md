...
- [ ] 3.0 자율신고 폼 및 제출 기능 구현 (Push 단위)
    ...
    - [ ] 3.2 위치 선택 컴포넌트 구현 (커밋 단위)
        - [ ] 3.2.1 `src/components/forms/LocationSelector.tsx` 생성
        - [ ] 3.2.2 주소 입력 필드 (텍스트 인풋)
        - [ ] 3.2.3 "현재 위치 사용" 버튼 (Geolocation API)
            - [ ] 3.2.3.1 navigator.geolocation.getCurrentPosition 호출
            - [ ] 3.2.3.2 위도/경도 자동 입력
            - [ ] 3.2.3.3 에러 처리 (위치 권한 거부)
            - [ ] 3.2.3.4 테스트 코드 작성
            - [ ] 3.2.3.5 테스트 실행 및 검증
            - [ ] 3.2.3.6 오류 수정 (필요 시)
        - [ ] 3.2.4 위도/경도 표시 (읽기 전용)
        - [ ] 3.2.5 지도 미리보기 (간단한 정적 지도 또는 플레이스홀더)
            - [ ] 3.2.5.1 테스트 코드 작성
            - [ ] 3.2.5.2 테스트 실행 및 검증
            - [ ] 3.2.5.3 오류 수정 (필요 시)
    ...
    - [ ] 3.6 Supabase RPC Function 구현 (submit_report) (커밋 단위)
        ...
        - [ ] 3.6.2 `submit_report` 함수 작성 (SQL)
            - [ ] 3.6.2.1 파라미터: location_name, location_lat, location_lng, activity_type, start_time, end_time, participants
            - [ ] 3.6.2.2 신고 ID 생성: 'RPT-' + YYYYMMDD + '-' + SEQ
            - [ ] 3.6.2.3 기본 안전도 계산 로직 (임시, 나중에 Edge Function으로 대체)
            - [ ] 3.6.2.4 reports 테이블에 INSERT
            - [ ] 3.6.2.5 JSONB 응답 반환
            - [ ] 3.6.2.6 테스트 코드 작성: RPC 호출 테스트
            - [ ] 3.6.2.7 테스트 실행 및 검증
            - [ ] 3.6.2.8 오류 수정 (필요 시)
        - [ ] 3.6.3 마이그레이션 적용: `supabase db push`
            - [ ] 3.6.3.1 테스트 코드 작성
            - [ ] 3.6.3.2 테스트 실행 및 검증
            - [ ] 3.6.3.3 오류 수정 (필요 시)

    - [ ] 3.7 신고 제출 API 구현 (커밋 단위)
        - [ ] 3.7.1 `src/app/api/report/submit/route.ts` 생성
        - [ ] 3.7.2 POST 핸들러 구현
            - [ ] 3.7.2.1 Supabase 클라이언트 생성 (createRouteHandlerClient)
            - [ ] 3.7.2.2 사용자 인증 확인 (getUser)
            - [ ] 3.7.2.3 요청 본문 파싱 및 검증
            - [ ] 3.7.2.4 Supabase RPC 호출: `supabase.rpc('submit_report', {...})`
            - [ ] 3.7.2.5 에러 처리 (401, 400, 500)
            - [ ] 3.7.2.6 성공 응답 반환
            - [ ] 3.7.2.7 테스트 코드 작성: API 엔드포인트 통합 테스트
            - [ ] 3.7.2.8 테스트 실행 및 검증
            - [ ] 3.7.2.9 오류 수정 (필요 시)

    - [ ] 3.8 신고 결과 페이지 구현 (커밋 단위)
        - [ ] 3.8.1 `src/app/report/[id]/page.tsx` 생성
        - [ ] 3.8.2 신고 ID로 데이터 조회 (Supabase SELECT)
            - [ ] 3.8.2.1 Server Component에서 데이터 페칭
            - [ ] 3.8.2.2 RLS 정책으로 본인 신고만 조회 가능
            - [ ] 3.8.2.3 테스트 코드 작성
            - [ ] 3.8.2.4 테스트 실행 및 검증
            - [ ] 3.8.2.5 오류 수정 (필요 시)
        - [ ] 3.8.3 신고 정보 표시
            - [ ] 3.8.3.1 신고 ID, 상태 (APPROVED/CAUTION/DENIED)
            - [ ] 3.8.3.2 위치 정보
            - [ ] 3.8.3.3 활동 정보
            - [ ] 3.8.3.4 안전도 점수 (색상 코드: GREEN/YELLOW/RED)
            - [ ] 3.8.3.5 테스트 코드 작성
            - [ ] 3.8.3.6 테스트 실행 및 검증
            - [ ] 3.8.3.7 오류 수정 (필요 시)
        - [ ] 3.8.4 안전 정보 섹션
            - [ ] 3.8.4.1 기상 상태 (목 데이터)
            - [ ] 3.8.4.2 어업권 정보 (목 데이터)
            - [ ] 3.8.4.3 항로 정보 (목 데이터)
            - [ ] 3.8.4.4 테스트 코드 작성
            - [ ] 3.8.4.5 테스트 실행 및 검증
            - [ ] 3.8.4.6 오류 수정 (필요 시)
        - [ ] 3.8.5 응급연락처 표시
            - [ ] 3.8.5.1 해양경찰 122, 119, 지역 관청
            - [ ] 3.8.5.2 테스트 코드 작성
            - [ ] 3.8.5.3 테스트 실행 및 검증
            - [ ] 3.8.5.4 오류 수정 (필요 시)

    - [ ] 3.9 신고 이력 페이지 구현 (커밋 단위)
        - [ ] 3.9.1 `src/app/report/history/page.tsx` 생성
        - [ ] 3.9.2 사용자의 모든 신고 조회
            - [ ] 3.9.2.1 Supabase SELECT WHERE user_id = auth.uid()
            - [ ] 3.9.2.2 최신순 정렬
            - [ ] 3.9.2.3 테스트 코드 작성
            - [ ] 3.9.2.4 테스트 실행 및 검증
            - [ ] 3.9.2.5 오류 수정 (필요 시)
        - [ ] 3.9.3 신고 목록 렌더링
            - [ ] 3.9.3.1 카드 형식으로 표시
            - [ ] 3.9.3.2 신고 ID, 날짜, 위치, 상태 표시
            - [ ] 3.9.3.3 클릭 시 상세 페이지로 이동
            - [ ] 3.9.3.4 테스트 코드 작성
            - [ ] 3.9.3.5 테스트 실행 및 검증
            - [ ] 3.9.3.6 오류 수정 (필요 시)
        - [ ] 3.9.4 빈 상태 처리 (신고 이력 없음)
            - [ ] 3.9.4.1 테스트 코드 작성
            - [ ] 3.9.4.2 테스트 실행 및 검증
            - [ ] 3.9.4.3 오류 수정 (필요 시)

    - [ ] 3.10 목 데이터 생성 (커밋 단위)
        - [ ] 3.10.1 `src/lib/data/mockData.ts` 생성
        - [ ] 3.10.2 안전구역 목 데이터 (부산 해운대, 강릉, 제주 등)
        - [ ] 3.10.3 기상 정보 목 데이터 (풍속, 파고, 시정)
        - [ ] 3.10.4 어업권 정보 목 데이터
        - [ ] 3.10.5 항로 정보 목 데이터
        - [ ] 3.10.6 테스트 코드 작성: 목 데이터 구조 검증
        - [ ] 3.10.7 테스트 실행 및 검증
        - [ ] 3.10.8 오류 수정 (필요 시)

    - [ ] 3.11 신고 서비스 로직 분리 (커밋 단위)
        - [ ] 3.11.1 `src/lib/services/reportService.ts` 생성
        - [ ] 3.11.2 `submitReport` 함수 (API 호출 래퍼)
        - [ ] 3.11.3 `getReportById` 함수
        - [ ] 3.11.4 `getReportHistory` 함수
        - [ ] 3.11.5 에러 처리 및 타입 안전성
        - [ ] 3.11.6 테스트 코드 작성: 서비스 함수 단위 테스트
        - [ ] 3.11.7 테스트 실행 및 검증
        - [ ] 3.11.8 오류 수정 (필요 시)

    - [ ] 3.12 E2E 테스트: 신고 제출 플로우 (커밋 단위)
        - [ ] 3.12.1 `__tests__/e2e/report-flow.spec.ts` 생성
        - [ ] 3.12.2 Playwright 테스트 작성
            - [ ] 3.12.2.1 로그인
            - [ ] 3.12.2.2 신고 폼 작성
            - [ ] 3.12.2.3 제출
            - [ ] 3.12.2.4 결과 페이지 확인
            - [ ] 3.12.2.5 이력 페이지에서 신고 확인
            - [ ] 3.12.2.6 테스트 실행: `npx playwright test`
            - [ ] 3.12.2.7 오류 수정 (필요 시)
