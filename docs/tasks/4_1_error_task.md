...
- [x] 2.0 Zod 종료 시간 검증 안정화 (상)
	...
	- [x] 2.2 ISO 변환/검증 유틸 정비 및 폼 스키마 업데이트
		...
		- [x] 2.2.2 테스트 실행 및 검증
		- [x] 2.2.3 오류 수정 (필요 시)
- [x] 3.0 Windy Point Forecast 400 대응 (상)
	- [x] 3.1 요청 payload/환경변수 점검 및 문서 비교
		- [x] 3.1.1 테스트 코드 작성
		- [x] 3.1.2 테스트 실행 및 검증
		- [x] 3.1.3 오류 수정 (필요 시)
	- [x] 3.2 API 실패 시 fallback 데이터 및 사용자 메시지 구현
		- [x] 3.2.1 테스트 코드 작성
		- [x] 3.2.2 테스트 실행 및 검증
		- [x] 3.2.3 오류 수정 (필요 시)
- [x] 4.0 공공데이터 경보 파서 TypeError 해결 (상)
	- [x] 4.1 `normalizeWarningTime` 입력 타입 가드 및 비정형 케이스 테스트
		- [x] 4.1.1 테스트 코드 작성
		- [x] 4.1.2 테스트 실행 및 검증
		- [x] 4.1.3 오류 수정 (필요 시)
- [x] 5.0 해경 관측소 API 404 대응 (중)
	- [x] 5.1 `coastGuardUrl` 파라미터/서비스키 검증 및 예외 핸들링
		- [x] 5.1.1 테스트 코드 작성
		- [x] 5.1.2 테스트 실행 및 검증
		- [x] 5.1.3 오류 수정 (필요 시)
- [x] 6.0 환경 인사이트 연쇄 실패 복원력 확보 (상)
	- [x] 6.1 `fetchEnvironmentalInsights` 개별 try-catch 및 UI degrade 시나리오 구현
		- [x] 6.1.1 테스트 코드 작성
		- [x] 6.1.2 테스트 실행 및 검증
		- [x] 6.1.3 오류 수정 (필요 시)
- [x] 7.0 Source map 파싱 경고 정리 (하)
	- [x] 7.1 문제 패키지 및 Turbopack 설정 점검, 필요 시 source map 비활성화
		- [x] 7.1.1 테스트 코드 작성
		- [x] 7.1.2 테스트 실행 및 검증
		- [x] 7.1.3 오류 수정 (필요 시)
- [x] 8.0 Leaflet `L` undefined 해결 (중)
	- [x] 8.1 Leaflet 의존 컴포넌트에 dynamic import 및 SSR 가드 추가
		- [x] 8.1.1 테스트 코드 작성
		- [x] 8.1.2 테스트 실행 및 검증
		- [x] 8.1.3 오류 수정 (필요 시)
- [x] 9.0 Supabase 세션 객체 보안 강화 (중)
	- [x] 9.1 `supabase.auth.getUser()` 기반 재검증 로직 도입 및 영향 범위 점검
		- [x] 9.1.1 테스트 코드 작성
		- [x] 9.1.2 테스트 실행 및 검증
		- [x] 9.1.3 오류 수정 (필요 시)
- [x] 10.0 Preload 미사용 경고 정리 (하)
	- [x] 10.1 Preload 링크 실사용 여부 검사 및 `as` 속성/타이밍 조정
		- [x] 10.1.1 테스트 코드 작성
		- [x] 10.1.2 테스트 실행 및 검증
		- [x] 10.1.3 오류 수정 (필요 시)
